...
import AOS from 'aos';
import { gstServices } from '../../data/gstServices.jsx';
import SEO from '../common/SEO';
import DynamicFaIcon from '../common/FontAwesomeRegistry';

const GSTServiceDetailPage = () => {
    const { serviceId } = useParams();
    const service = gstServices[serviceId];
    ...
    if (!service) {
        return <Navigate to="/services/gst" replace />;
    }
    ...
    return (
        <main className="font-inter text-white bg-dark-bg pt-24 md:pt-28">
            ...
            <section className="container mx-auto px-4 md:px-20 mb-12" data-aos="fade-up">
                <Link to="/services/gst" className="inline-flex items-center gap-2 text-primary-accent text-sm mb-4 hover:underline">
                    <DynamicFaIcon name="fa-arrow-left" /> Back to GST Services
                </Link>
                <div className="bg-secondary-dark border border-primary-accent/20 rounded-3xl p-8 md:p-12 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-8 opacity-5 md:opacity-10">
                        <DynamicFaIcon name={service.icon} className="text-9xl text-primary-accent" />
                    </div>
                            ...
                            <div className="mt-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {service.subServices.map((sub, idx) => (
                                        <Link key={idx} to={`/services/gst/${sub.slug}`} className="block bg-secondary-dark/50 border border-primary-accent/10 hover:bg-secondary-dark hover:border-primary-accent/40 p-5 rounded-xl transition-all group">
                                            ...
                                            <div className="mt-3 text-xs text-primary-accent/60 group-hover:text-primary-accent flex items-center gap-1">
                                                <span>Read More</span> <DynamicFaIcon name="fa-arrow-right" />
                                            </div>
                                        </Link>
                                    ))}
                                </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-secondary-dark/80 p-6 rounded-2xl border border-primary-accent/10 sticky top-28">
                            ...
                            <p className="text-sm text-boulder mb-6">
                                Our experts are ready to assist you. Get compliant today.
                            </p>
                            <a
                            ...
                            >
                                <DynamicFaIcon name="fa-whatsapp" className="mr-2 text-lg" /> Chat on WhatsApp
                            </a>
                            <Link
                                to="/contact"
            ...
            <section className="container mx-auto px-4 md:px-20 mb-16 pb-10 border-t border-primary-accent/10 pt-10">
                <h3 className="text-xl font-bold text-white mb-6">Explore Other GST Services</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {Object.entries(gstServices).slice(0, 4).map(([key, item]) => {
                        if (key === serviceId) return null;
                        return (
                            <Link key={key} to={`/services/gst/${key}`} className="block bg-secondary-dark border border-primary-accent/5 hover:border-primary-accent/30 p-4 rounded-xl transition-all">
...
export default GSTServiceDetailPage;
